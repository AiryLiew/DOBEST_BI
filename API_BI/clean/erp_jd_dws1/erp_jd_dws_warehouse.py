...
df_warehouse= pd.read_sql_query(text("""select riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(pankuisl,SIGNED) shipping,company,case when wuliaomc is not null then '盘亏单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_inventoryloss 
									...
									union all
                                    select riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(panyingsl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '盘盈单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_inventoryprofit 
                                    where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    and riqi>=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 63 DAY), '%Y-%m-01')
									...
									union all
                                    select riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(shifasl,SIGNED) shipping,company,case when wuliaomc is not null then '其他出库单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_othersshipping 
									where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and shenhezt = '已审核'
                                    and riqi>=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 63 DAY), '%Y-%m-01')


                                    union all
                                    select riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(-shifasl,SIGNED) shipping,company,case when wuliaomc is not null then '销售退货单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_salereturn 
									where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and shenhezt = '已审核'
                                    and riqi>=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 63 DAY), '%Y-%m-01')

                                    
									union all
                                    select riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(shifasl,SIGNED) shipping,company,case when wuliaomc is not null then '销售出库单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_saleshipping 
									where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and shenhezt = '已审核'
                                    and riqi>=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 63 DAY), '%Y-%m-01')


                                    union all
                                    select rukurq riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shuliang,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '组装拆卸单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_assemble 
									...
									union all 
                                    select rukurq riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(shuliang,SIGNED) shipping,company,case when wuliaomc is not null then '组装拆卸单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_assemble 
...
df_warehouse_s = pd.read_sql_query(text("""SELECT a.riqi,	a.wuliaomc,a.wuliaobm,a.cangkumc,a.cangkuid	,a.receiving,a.shipping	,a.company,a.table,	a.inventory_wl inventory
                                            FROM (  
                                                SELECT *,   
                                                    ROW_NUMBER() over(partition by  wuliaomc order by riqi desc, inventory_wl) as rankw   
...
df_warehouse['标记'] = 'keep'
df_warehouse_s['标记'] = 'drop'

df_warehouse = pd.concat([df_warehouse,df_warehouse_s])
...
df_warehouse = df_warehouse[df_warehouse['标记'] == 'keep'].drop(['标记'],axis = 1)
