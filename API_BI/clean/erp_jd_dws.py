...
df_purchasereturn    = pd.read_sql_query(text('select * from erp_jd_dwd.erp_jd_dwd_dim_purchasereturn;'), engine.connect())
df_purchaseorders    = pd.read_sql_query(text('select * from erp_jd_dwd.erp_jd_dwd_dim_purchaseorders;'), engine.connect())
df_saleorders        = pd.read_sql_query(text('select * from erp_jd_dwd.erp_jd_dwd_dim_saleorders;'), engine.connect())
df_consignment       = pd.read_sql_query(text('select * from erp_jd_dwd.erp_jd_dwd_dim_consignment;'), engine.connect())
df_saleship_return   = pd.read_sql_query(text('select * from erp_jd_dws.erp_jd_dws_saleship_return;'), engine.connect())
...
df_warehouse= pd.read_sql_query(text("""select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(pankuisl,SIGNED) shipping,company,case when wuliaomc is not null then '盘亏单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_inventoryloss where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all
                                    ...
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(shifasl,SIGNED) shipping,company,case when wuliaomc is not null then '其他出库单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_othersshipping where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and shenhezt = '已审核'
                                    ...
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,cangkumc,cangkuid,case when wuliaomc is not null then 0 end receiving,CONVERT(shifasl,SIGNED) shipping,company,case when wuliaomc is not null then '销售出库单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_saleshipping where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and shenhezt = '已审核'
                                    ...
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,diaochuck cangkumc,diaochuckid cangkuid,CONVERT(-diaorusl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '分布式调入单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_distributedin where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and danjuzt = '已审核'
                                    union all 
                                    ...
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,diaochuck cangkumc,diaochuckid cangkuid,CONVERT(diaochusl,SIGNED) receiving,CONVERT(diaochusl,SIGNED) shipping,company,case when wuliaomc is not null then '分布式调出单' end `table` from erp_jd_dwd.erp_jd_dwd_dim_distributedout where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签') and danjuzt = '已审核'
                                    ...
                                    a.`table` 
                                    from (
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_wc_dobest where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_wc_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_ms_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_yc_xmgs where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_yc_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(tuiliaorq,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(-shituisl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购退料单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereturn_kyk_cwzx  where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')

                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_wc_dobest where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_wc_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_ms_dobest where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_ms_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_yc_xmgs where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_yc_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    union all 
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(shifasl,SIGNED) receiving,case when wuliaomc is not null then 0 end shipping,company,case when wuliaomc is not null then '采购入库单' end `table` from erp_jd_ods.erp_jd_ods_dim_purchasereceiving_kyk_cwzx where wuliaomc not in ('代收运费','测试物料1','管易云运费','激光标签-icon版','防伪贴','塑封膜','盲盒方形防伪标签','盲盒圆形防伪标签')
                                    
                                    union all
                                    select STR_TO_DATE(riqi,'%%Y-%%m-%%d') riqi,wuliaomc,wuliaobm,cangkumc,cangkuid,CONVERT(receiving,SIGNED) receiving,CONVERT(shipping,SIGNED) shipping,company,`table` from erp_jd_dwd.erp_jd_dwd_dim_beginninginventory
                                    ) a
                                    LEFT JOIN(
...
def dayend(df_warehouse):
    ...
    df_warehouse_dayend = df_warehouse.groupby(['wuliaomc','riqi'],as_index=False).agg({'receiving':'sum','shipping':'sum','inventory':'sum'})
    df_warehouse_dayend.sort_values(['wuliaomc','riqi'],ascending=True,inplace=True,ignore_index=True)
    df_warehouse_dayend1 = df_warehouse_dayend.set_index(['riqi','wuliaomc']).groupby(['wuliaomc'])['inventory'].cumsum().reset_index().rename(columns = {'inventory':'inventory_wl'})
    ...
    df_warehouse_dayend['ismonthend'].fillna('否',inplace=True)
...
"INSERT INTO erp_jd_dws_warehouse_dayend(riqi,wuliaomc,receiving,shipping,inventory,inventory_wl,`year_month`,ismonthend,refresh) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)")
...
"INSERT INTO dlzy_inventory_dayend(riqi,wuliaomc,receiving,shipping,inventory,inventory_wl,`year_month`,ismonthend,属性) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)")
